    ...
    if(  isset( $_GET["submit"]  ) )
    {
        ...
        if ($uploadOk == 1) {
            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                ...
                $s_gender = trim($_GET["gender"]);
                $s_branch = trim($_GET["branch"]);
                $s_password = trim($_GET["password"]);

                $enc_password = password_hash($s_password , PASSWORD_DEFAULT   );

                $myQuery = $conn->prepare("INSERT INTO student(fullname, semail, contact, gender,branch, password) VALUES(?,?,?,?,?,?)");

                $myQuery->bind_param("ssssss",$f_name,$s_email,$s_contact,$s_gender,$s_branch,$enc_password,);
